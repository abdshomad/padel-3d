...
function ControlPanel({ design, onDesignChange, onAiGenerate, isLoading, error, onReset }: ControlPanelProps) {
  ...
  return (
    <aside className="w-full lg:w-96 bg-gray-800/80 backdrop-blur-sm p-6 pt-24 lg:pt-6 space-y-6 overflow-y-auto z-10 border-r border-gray-700">
      <div>
        <h2 className="text-xl font-semibold mb-4 text-blue-300">{t('aiDesignerTitle')}</h2>
        <div className="space-y-3">
          ...
          >
            {isLoading ? <Loader /> : <Wand2 className="w-5 h-5" />}
            {isLoading ? t('generating') : t('generateDesign')}
          </button>
      ...
      <div className="border-t border-gray-700 pt-6">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-blue-300">{t('manualControlsTitle')}</h2>
            <button onClick={onReset} className="p-2 rounded-full hover:bg-gray-700 transition-colors" title={t('resetToDefault')}>
                <RotateCw className="w-5 h-5" />
            </button>
        </div>
        <div className="space-y-4">
          <ColorInput label={t('courtColor')} value={design.courtColor} onChange={(e) => onDesignChange({ courtColor: e.target.value })} />
          <ColorInput label={t('linesColor')} value={design.linesColor} onChange={(e) => onDesignChange({ linesColor: e.target.value })} />
          <ColorInput label={t('outOfPlayColor')} value={design.outOfPlayColor} onChange={(e) => onDesignChange({ outOfPlayColor: e.target.value })} />
          <ColorInput label={t('frameColor')} value={design.frameColor} onChange={(e) => onDesignChange({ frameColor: e.target.value })} />
          <ColorInput label={t('netColor')} value={design.netColor} onChange={(e) => onDesignChange({ netColor: e.target.value })} />
          <ColorInput label={t('netLogoColor')} value={design.logoColor} onChange={(e) => onDesignChange({ logoColor: e.target.value })} />

          <div>
            <label className="text-sm text-gray-300 block mb-2">{t('glassOpacity')}: {Math.round(design.glassOpacity * 100)}%</label>
            <input
              type="range"
              min="0.05"
              max="0.5"
